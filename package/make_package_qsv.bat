...
if exist "Aviutl_%ENCODER_NAME%_%ENCODER_VERSION%.zip" del "Aviutl_%ENCODER_NAME%_%ENCODER_VERSION%.zip"
REM if exist "%ENCODER_NAME%_%ENCODER_VERSION%_7zip.7z" del "%ENCODER_NAME%_%ENCODER_VERSION%_7zip.7z"
REM start "7-zipà≥èk" /b "%SEVENZIP_PATH%" a -t7z -mx=8 -mmt=off "%ENCODER_NAME%_%ENCODER_VERSION%_7zip.7z" "%CUR_DIR%\%ENCODER_NAME%_%ENCODER_VERSION%"
